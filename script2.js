window.addEventListener("load", function () {
    ...
    const html = document.documentElement; // Selectăm <html>
    const body = document.body; // Selectăm <body>
    ...
    const interval = setInterval(() => {
        progress += 1; // Crește progresul cu 1%
        loadingPercentage.textContent = `${progress}%`; // Actualizează procentul

        if (progress >= 100) {
            clearInterval(interval); // Oprește intervalul la 100%

            // Adaugă clasa hidden pentru fade-out
            preloader.classList.add("hidden");

            // Așteaptă ca tranziția CSS să se finalizeze (0.8s)
            preloader.addEventListener(
                "transitionend",
                () => {
                    preloader.style.display = "none"; // Elimină complet coperta
                    videoBack.play(); // Pornește videoclipul
                    html.classList.remove("no-scroll"); // Reactivează scroll-ul
                    body.classList.remove("no-scroll"); // Reactivează scroll-ul
                },
                { once: true } // Asigură-te că funcția se execută o singură dată
            );
        }
    }, 20); // Crește progresul la fiecare 20ms
});
