...
window.addEventListener("load", () => {
    ...
    videoBack.currentTime = 0; // Resetează videoclipul la început
    videoBack.pause(); // Pornește videoclipul principal
...
// Selectăm elementele video de controlat (de exemplu: #videoback și #videoaer2)
const videoElements = document.querySelectorAll("#videoback, #videoaer2, #videoaer");

// Creăm un observer care verifică dacă cel puțin 50% din element este vizibil
const videoObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      entry.target.play();
      console.log(`Video-ul cu id-ul "${entry.target.id}" rulează.`);
    } else {
      entry.target.pause();
      console.log(`Video-ul cu id-ul "${entry.target.id}" este pe pauză.`);
    }
  });
}, { threshold: 0.2 });

// Atașăm observer-ul la fiecare element video
videoElements.forEach(video => videoObserver.observe(video));


document.addEventListener("DOMContentLoaded", function () {
    const nevoieText = document.getElementById("nevoie");
    const titluProiecteText = document.getElementById("titluproiecte");
    const verticalText = document.getElementById("vidvertical");
    const stabText = document.getElementById("stabilizare");
    const titlu1Text = document.getElementById("titlu1");
    const highendText = document.getElementById("highend");
    const colabText = document.getElementById("colab");
    ...
    observer.observe(nevoieText);
    observer.observe(titluProiecteText);
    observer.observe(verticalText);
    observer.observe(stabText);
    observer.observe(titlu1Text);
    observer.observe(highendText);
    observer.observe(colabText);
});


document.addEventListener("DOMContentLoaded", function () {
    const colaboratori = document.querySelector(".colaboratori");
    const duplicat = colaboratori.cloneNode(true);
...
gsap.to("#mini4pro", {
    ...
    scrollTrigger: {
        ...
        start: "top center 30%", // Începe când descriere1 ajunge în centrul viewport-ului
        end: "bottom center", // Termină la sfârșitul descriere1
        scrub: 2, // Sincronizare lină
      pin:true,
      batchMax: 3 // Optimizează numărul de elemente animate simultan
...
// Selectează toate span-urile din #highend-description
const highendWords = document.querySelectorAll("#highend-description span");

// Aplică animația pentru fiecare cuvânt, pe rând
highendWords.forEach((word, index) => {
    gsap.fromTo(
        word,
        { color: "gray", opacity: 0.5 }, // Stare inițială
        {
            color: "#000000", // Culoare finală (negru)
            opacity: 1, // Devine complet vizibil
            scrollTrigger: {
                trigger: "#highend-description", // Declanșează animația pentru text
                start: `top+=${index * 3}% 85%`, // Activează fiecare cuvânt treptat
                end: `top+=${index * 3}% 75%`, // Termină puțin mai jos
                scrub: 0.5, // Sincronizare lină cu scroll-ul
                markers: false, // Dezactivează marcajele pentru debug
            }
        }
    );
});
...
gsap.fromTo(
    ...
    { x: "100%", opacity: 0 }, // Începe din afara ecranului, partea dreaptă
    {
        ...
        ease: "power2.out", // Tranziție lină
        scrollTrigger: {
            trigger: "#RS5", // Declanșează animația când #RS4 intră în viewport
            start: "center 70%", // Începe când secțiunea este aproape de marginea de jos a ecranului
            end: "top ", // Termină când ajunge la mijlocul viewport-ului
            scrub: 2, // Sincronizare lină cu scroll-ul
        }
...
gsap.fromTo(
    ...
    { x: "-100%", opacity: 0 }, // Începe din afara ecranului, partea stângă
    {
        x: "-20%", // Ajunge la poziția inițială
        opacity: 1, // Devine vizibil
        ease: "power2.out", // Tranziție lină
        scrollTrigger: {
            trigger: ".hexagonz", // Declanșează animația când hexagonul intră în viewport
            start: "top 80%", // Începe când hexagonul e aproape de marginea de jos a ecranului
            end: "top 50%", // Termină când ajunge la mijlocul viewport-ului
            scrub: 2, // Sincronizare lină cu scroll-ul
        }
...
document.addEventListener("DOMContentLoaded", function () {
    const videos = [
        ...
        { video: document.getElementById("pr4"), toggle: document.getElementById("toggle4") },
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const clone = carousel.innerHTML; // Clonează imaginile pentru un efect continuu
    carousel.innerHTML += clone; // Adaugă imaginile duplicate pentru o tranziție lină
});
